...
void loop() {
  ...
  if(sensorValue < 300 && flag == true){ 
    ...
    if(timeInSeconds > 0.3 && timeInSeconds < 1.0){       //If time is > 0.3s and < 1s, a "1" is added to the String
      morse.dash();
    }
    if(timeInSeconds > 1.0){                              //If time is > 1s, morse code letter is printed to LCD
